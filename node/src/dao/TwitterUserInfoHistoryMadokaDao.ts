 ...
 * まどか Twitter ユーザー履歴
 */
export default class TwitterUserInfoHistoryMadokaDao {
	...
	async getDbh(): Promise<sqlite.Database<sqlite3.Database, sqlite3.Statement>> {
		...
		const dbh = await sqlite.open({
			filename: this.#config.sqlite.db.madokatwitter,
	 ...
	 */
	async selectUsers(): Promise<MadokaTwitterDb.User[]> {
		...
		const datas: MadokaTwitterDb.User[] = [];
	 ...
	 */
	async updateUsers(data: MadokaTwitterDb.User): Promise<void> {
		const dbh = await this.getDbh();
	 ...
	 */
	async selectLatestProfileImage(id: string): Promise<MadokaTwitterDb.ProfileImage | null> {
		const dbh = await this.getDbh();
	 ...
	 */
	async insertProfileImage(data: MadokaTwitterDb.ProfileImage): Promise<void> {
		const dbh = await this.getDbh();
	 ...
	 */
	async selectLatestBanner(id: string): Promise<MadokaTwitterDb.Banner | null> {
		const dbh = await this.getDbh();
	 ...
	 */
	async insertBanner(data: MadokaTwitterDb.Banner): Promise<void> {
		const dbh = await this.getDbh();
